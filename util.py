...
def listYearbook(train=True, valid=True):
  ...
  s = []
  if train: 
    r = r + [n.strip() for n in open(YEARBOOK_PATH+'_train.txt','r')]
    s = s + [label(n.strip()) for n in open(YEARBOOK_PATH+'_train.txt','r')]
  if valid: 
    r = r + [n.strip() for n in open(YEARBOOK_PATH+'_valid.txt','r')]
    s = s + [label(n.strip()) for n in open(YEARBOOK_PATH+'_valid.txt','r')]
  return r,s
